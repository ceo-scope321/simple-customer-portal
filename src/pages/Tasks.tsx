...
const sampleTasks: Task[] = [
  {
    ...
    description: "Discuss the scope and timeline for the Acme Corp website redesign",
    dueDate: "2023-06-10",
    priority: "high",
    status: "todo",
    assignee: "John Doe",
  ...
  },
  {
    ...
    description: "Coordinate a time for the product training with Bright Solutions",
    dueDate: "2023-06-12",
    priority: "medium",
    status: "todo",
    assignee: "John Doe",
...
export default function Tasks() {
  ...
  const filteredTasks = tasks.filter(task => {
    switch (activeTab) {
      ...
      case 'upcoming':
        return task.dueDate && formatDate(task.dueDate) !== 'Today' && !isOverdue(task.dueDate);
      case 'overdue':
          ...
          {tasks.length === 0 ? (
            <div className="flex items-center justify-center h-64">
              <div className="text-center animate-pulse">
                ...
                <p className="text-sm text-muted-foreground">Please wait while we fetch your tasks</p>
              </div>
            </div>
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1">
                        ...
                        <div className="flex flex-wrap gap-4 mt-3 text-xs text-muted-foreground">
                          {task.dueDate && (
                            <div className={`flex items-center ${isOverdue(task.dueDate) && task.status !== 'done' ? 'text-red-500' : ''}`}>
                          ...
                          {task.relatedTo && (
                            <div className="flex items-center">
                              <Badge variant="outline" className="h-5 text-xs font-normal">
                                {task.relatedTo.type === 'customer' ? 'Customer' : 'Deal'}: {task.relatedTo.name}
                              </Badge>
                            </div>
                          )}
                        </div>
